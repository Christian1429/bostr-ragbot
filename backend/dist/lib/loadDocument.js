...
export const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (file.mimetype === 'application/json' ||
            ...
            cb, (new Error('Endast PDF- och JSON-filer är tillåtna!'), false);
        }
    },
});
export const loadDocuments = (req, res) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (sourceType === 'pdf' && req.file) {
                ...
                documentContent = await ExtractTextFromJson(Array.isArray(jsonData) ? jsonData : [jsonData]);
            }
            ...
            await addToVectorStore(documentContent, sourceType, sourceType === 'url' ? body.url || '' : '');
            res.json({
